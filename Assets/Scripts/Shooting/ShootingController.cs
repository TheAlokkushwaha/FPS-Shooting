...
public class ShootingController : MonoBehaviour
{
    ...
    void HandleFiring()
    {
        bool isShooting = (isAuto && Input.GetButton("Fire1")) || (!isAuto && Input.GetButtonDown("Fire1"));
    ...
    private void Shoot()
    {
        if (currentAmmo > 0)
        {
            ...
            if (Physics.Raycast(firePoint.position, firePoint.forward, out hit, fireRange))
            {
                ...
                WaypointZobieAI waypointzombieAI = hit.collider.GetComponent<WaypointZobieAI>();
